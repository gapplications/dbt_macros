{% macro timeframe_product_summary(time_granularity, product_lifecycle_tables, granularity) %}
...
SELECT
    date_spine.date_{{time_granularity}}
    {% for cte in cte_list %}
    , {{ cte }}.* EXCEPT( date_{{ time_granularity }})
    {% endfor %}

FROM date_spine
    {% for cte in cte_list %}
    LEFT JOIN {{ cte }}
        ON date_spine.date_{{time_granularity}} = {{ cte }}.date_{{ time_granularity }}
    {% endfor %}
